...
result *add_artist(char *name) {
    ...
    strcpy(new_artist->name, name); //TODO: find out why this line may not work
...
//         printf("%s\n", current_artist->name);
    while(current_artist) {
        if(strcmp(current_artist->name, name) == 0) { // TODO: find out why this line doesn't work
...
int main(int argc, char **argv) {
    ...
    do {
        printf("Interpret/Album (h)inzufuegen, (a)nzeigen, (e)ntfernen, (s)uchen: ");
        scanf(" %c", &menu);
        switch(menu) {
            case 'H':
            case 'h':
    ...
    } while(menu == 'H' || menu == 'h' || menu == 'A' || menu == 'a' || menu == 'E' || menu == 'e' || menu == 'S' || menu == 's');
